...
simpletheme_blankx <- c(simpletheme,
...
getgloms <- function(ps,ranks= c("Phylum" ,"Class","Order","Family","Genus","Species")){
...
plot_taxa_abundance <- function(ps,rank,xsep, wrap = NULL, n=20, colno = NULL,byabundance=TRUE,glom=NULL){

  #set cols
  cols.n <- c(c(nice20, ridiculouslybigcolset)[1:n],"lightgrey")

  #check if agglomerated object provided. If not - agglomerate.
  if(is.null(glom)){
    print("No agglomerated object provided. Agglomerating...")
    ps@tax_table@.Data[is.na(ps@tax_table@.Data)] <- "Unassigned"
...
brayplot <- function(ps, pcoa, colour, shape=NULL){
...
alphaplot <- function(richtable, xvar, yvar, wrapvar = NULL, colno = NULL){
